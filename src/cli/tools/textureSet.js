 ...
 * @returns {Promise<Object>} - 返回一个对象，键为文件名，值为相对路径
 */
  ...
  for (const file of files) {
    ...
    if (fileStats.isDirectory()) {
      ...
      texturesSet[fileName] = {
        textures: `${prefix}${relativePath.replace(/\\/g, "/")}`, // 确保路径使用正斜杠
      };
    }
  }
...
export async function generateBlockTextureJson(itemTexturesPath, outputPath, projectName) {
  try {
    ...
    const itemTexturesSet = await traverseDirectory(itemTexturesPath, itemTexturesPath);
